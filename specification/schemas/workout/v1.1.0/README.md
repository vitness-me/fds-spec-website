...
Worked examples for `workout.schema.json` (RFC-007), at **v1.1.0**. Every one is a valid 1.0.0 document too, except where a fixture demonstrates a 1.1.0 addition — the version is purely additive. Each is a complete, validating workout, and each demonstrates one row of the scenario coverage matrix.
...
| `workout.machine-settings.example.json` | Incline climbing per set, then cadence with a tolerance band. Shows the 1.1.0 additions doing the work they exist for, and shows the division: resistance is a *load* (`loadTarget.method: "level"` with a named scale), incline and cadence are *settings* the athlete dials in. |
